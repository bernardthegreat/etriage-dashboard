				...
				<div class="row">
						<div class="col-sm-6"  v-bind:key="key" v-for="(app, key) in apps">
								<div class="position-relative rounded" v-bind:class="[app.background]"
										...
										<a href="#" v-on:click.prevent="changeRoute(app.route)"
...
export default {
	...
	data() { 
			return {
					...
					apps: [
							{ name: "Employees / Residents / Consultants", route: "/employees", icon: "fa fa-user", background: "bg-primary", count: '' },
							{ name: 'Visitors', route: '/visitors', icon: "fa fa-users", background: "bg-danger", count: '' },
					],
					isDataLoaded: false,
			}
	},
