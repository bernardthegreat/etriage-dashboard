<template>
    <div class="container py-5" >
		...
		<div v-if="isDataLoaded">
			...
			<div class="card-deck mb-12 text-center">
				<div class="card mb-4 shadow-sm" v-bind:key="key" v-for="(eTriageByGenderCount, key) in eTriageByGenderCountToday">
					<div class="card-header">
						<h4 class="my-0 font-weight-normal">
							<span v-if="eTriageByGenderCount['gender'] === 'N/A'">OTHERS</span>
							<span v-else-if="eTriageByGenderCount['gender'] === 'F'">FEMALE</span>
							<span v-else-if="eTriageByGenderCount['gender'] === 'M'">MALE</span>
						</h4>
					</div>
					<div class="card-body">
						<h1 class="card-title pricing-card-title">{{eTriageByGenderCount['count']}}</h1>

						<button
							type="button"
              v-on:click.prevent="changeRoute('/eTriageByGenderToday')"
							class="btn btn-lg btn-block btn-info"
						>See Detailed Data</button>
					</div>
				</div>
			</div>

			<div class="card-deck mb-12 text-center">
				<div class="card mb-4 shadow-sm" v-bind:key="key" v-for="(eTriageByClassCount, key) in eTriageByClassCountToday">
					<div class="card-header">
						<h5 class="my-0 font-weight-normal">{{eTriageByClassCount['class']}}</h5>
					</div>
					<div class="card-body">
						<h1 class="card-title pricing-card-title">{{eTriageByClassCount['count']}}</h1>

						<button type="button" class="btn btn-lg btn-block btn-warning">See More</button>
					</div>
				</div>
			</div>
				...
				<div class="card">
					...
					<div class="card-body">
						<div style="height: 400px; overflow-y: auto;">
							<table id="table-table" class="table table-striped table-hover text-center">
								<thead>
									<tr>
								...
								<tbody>
									<tr
										id="results"
										v-bind:key="key" v-for="(eTriageHistoricalResult, key) in eTriageHistoricalFilter"
										v-bind:class="[eTriageHistoricalResult.isForCovidEr === 1 ? 'font-weight-bold text-red' : '']"
									>
...
export default {
	...
	methods: {
		...
		filterHistory(name) {
				...
				this.eTriageHistoricalFilter = this.eTriageHistoricalFilter.filter((item) => item.name == name)
		...
		async eTriageHistoricalData() {
				...
				for (var result of responseJson) {
						...
						let formatted_date = dt.getFullYear() + "-" + (dt.getMonth() + 1) + "-"
								+ dt.getDate() + " " + hours + ":" + minutes + AmOrPm;
